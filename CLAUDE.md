...
- **Onboarding**: Welcome (Sign in with Apple) → Profile setup (username, checked for case-insensitive uniqueness) → Identity verification (phone number + Stripe Identity document/selfie scan) → Welcome/take-first-photo → main app. Progress persists to `users.onboarding_step` after every step, so force-quitting mid-onboarding resumes at the right screen on relaunch. **Identity verification is optional** — the `.verify` step is headed "Verify your identity (optional)" and offers Skip alongside Verify Identity; both advance to the next step, and skipping leaves `verification_status` at `unverified`. Someone who skipped can start it later from Settings (see below). Identity verification is real, via **Stripe Identity** (`Onboarding/IdentityVerificationView.swift` + `Onboarding/StripeIdentityWebView.swift`, a WKWebView presenting Stripe's hosted flow) — **document + selfie checks only, no SSN check requested**; no field for an SSN exists in any model or table, by design (explicit user requirement). The Stripe secret key lives only in Supabase Edge Functions in `the-human-internet-backend`, never client-side — see "Stripe Identity" below.
...
- **Starting it after a skip**: Settings shows an "Identity Verification / Verify Identity" row **only while `verification_status` is `unverified`**, which presents the same `IdentityVerificationView` onboarding uses (with "Not now" in place of "Skip"). It's deliberately not offered at `in_progress` — that user is waiting on the webhook, and restarting would orphan their session.
...
- Current flags, and **each falls back in its own direction** when the server has nothing this build can use — no row, flags not loaded, or an unrecognised audience. The fallbacks live in one place, `FeatureFlagKey.fallbackAudience(for:)`; an unknown key falls back to `off`:
  - `stripe_identity_verification` (`FeatureFlagKey.stripeIdentityVerification`) — **currently `off`.** Off ⇒ `OnboardingFlowView`'s `.verify` step skips `IdentityVerificationView` entirely and advances straight through, leaving `verification_status` at `unverified` — both `HumanUser`'s Swift default and the column default, since verification became optional — the mitigation for Stripe Identity being blocked, see Known gaps below. Now that the step is skippable anyway, this flag only removes the *choice*, not the requirement. Falls back to **`all`**, so a fetch glitch can't silently remove the option.
