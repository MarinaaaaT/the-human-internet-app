...
- **`verification_status` is the one column that changes behind the app's back**, so it's the one thing `hydrate` alone can't keep current. `stripe-identity-webhook` writes it with the service role while the app is running, and nothing pushes that down — before `AppState.refreshVerificationStatus()` existed, a verification that resolved mid-session only became visible after a force-quit and relaunch. It's re-read on foreground (`RootView`'s `scenePhase` change) and on opening Settings, which then polls every 5s *only* while the status is `in_progress` — the sole status that resolves on its own. Deliberately a one-column read rather than a `hydrate`: hydrate refetches photos and would briefly drop optimistically-inserted pending captures, and assigning a whole fetched row over `AppState.user` would stomp whatever `EditUsernameSheet`/`PrivacyEditSheet` has half-edited in place.
...
- **Onboarding**: Welcome (Sign in with Apple) → Profile setup (username, checked for case-insensitive uniqueness) → Identity verification (Stripe Identity document/selfie scan) → Welcome/take-first-photo → main app. Progress persists to `users.onboarding_step` after every step, so force-quitting mid-onboarding resumes at the right screen on relaunch. **Identity verification is optional** — the `.verify` step is headed "Verify your identity (optional)" and offers Skip alongside Verify Identity; both advance to the next step, and skipping leaves `verification_status` at `unverified`. Someone who skipped can start it later from Settings (see below). Identity verification is real, via **Stripe Identity** (`Onboarding/IdentityVerificationView.swift` + `Onboarding/StripeIdentityWebView.swift`, a WKWebView presenting Stripe's hosted flow) — **document + selfie checks only, no SSN check requested**; no field for an SSN exists in any model or table, by design (explicit user requirement). The Stripe secret key lives only in Supabase Edge Functions in `the-human-internet-backend`, never client-side — see "Stripe Identity" below.
...
- **Settings**: edit username (live uniqueness check against a DB constraint, friendly conflict error), edit privacy, verification-status info sheet (copy varies per status), a **Verify Identity** row shown only while `verification_status` is `unverified`, **Log Out** (confirmation dialog → `AppState.signOut()` → back to Welcome).
...
- **iOS** ([Onboarding/IdentityVerificationView.swift](the-human-internet/the-human-internet/Onboarding/IdentityVerificationView.swift), [Onboarding/StripeIdentityWebView.swift](the-human-internet/the-human-internet/Onboarding/StripeIdentityWebView.swift)): the app never talks to Stripe directly. It calls the `stripe-identity-session` Edge Function via `supabase.functions.invoke` (the `Functions` product from `supabase-swift` was already linked but unused before this), gets back a hosted verification URL, and presents it in a `WKWebView` modal — not Stripe's native `StripeIdentity` SDK, to avoid a 7th manual `.pbxproj` SPM edit site for one screen. Completion is detected by intercepting a redirect to the VerificationSession's `return_url`, **`https://the-human-internet.com/identity-verification-return`** — recognised by `StripeIdentityHostPolicy.isReturnURL`, which cancels the navigation before it loads, so nothing needs to exist at that path on the website. It is deliberately *not* the app's `thehumaninternet://` scheme: **Stripe validates `return_url` and rejects a custom scheme with `url_invalid`**, which 400s session creation before it starts (this is exactly what happened the first time the flow was run for real, on 2026-08-29, and it surfaced in the app only as "Couldn't start verification"). The constant is mirrored in the `stripe-identity-session` Edge Function and drift between the two halves is silent — Stripe finishes and the web view simply never dismisses — so `StripeIdentityHostPolicyTests` pins the app's half. The old custom-scheme URL is still recognised, purely so a session minted before the change still completes.
- **Two Stripe environments, chosen server-side.** `stripe-identity-session` decides live vs test from the `stripe_identity_test_mode` flag *and* the caller's `users.is_admin`, both re-read with the caller's JWT — the app's `isStripeIdentityTestModeEnabled` only drives what the UI shows (a warning banner on the verify screen, and the environment line under the dev menu's Flow Trigger). One webhook endpoint serves both: it tries `STRIPE_WEBHOOK_SECRET` then `STRIPE_WEBHOOK_SECRET_TEST` and the secret that verifies identifies the environment, after which `event.livemode` is trustworthy because it arrived inside a signature-checked payload. Switching environments makes the stored `stripe_identity_session_id` unresolvable in the new one — that retrieve is expected to 404 and falls through to creating a fresh session rather than failing the request.
...
- **What the `verification_status` values *mean*.** Since verification is optional, `unverified` (never started, or deliberately skipped) is a legitimate resting state — *not* a stage on the way to `verified`. `in_progress` means specifically "submitted to Stripe, awaiting the result", set client-side when Stripe redirects back and resolved only by the `stripe-identity-webhook` Edge Function. Anything reading this must treat `unverified` and `in_progress` as different things; conflating them is what the old three-value model got wrong.
- **`verification_status` is write-protected, and the rules are not obvious.** RLS on `users` is row-scoped but *not* column-scoped, and the policy lets a user write their own full row — so until this was guarded, any authenticated user could `PATCH /rest/v1/users` and set themselves to `verified`. That is the one claim the entire product rests on, so two triggers now mirror the `is_admin` pattern below:
  - `prevent_self_verification_escalation` (BEFORE UPDATE) reverts any change to the column, **except** the single transition `unverified → in_progress`, which is the one write the app itself performs (`IdentityVerificationView`, when Stripe redirects back after submission — it claims strictly *less* about a user than `verified` does). Everything else, above all anything reaching `verified`, is reverted.
  - `force_unverified_on_insert` (BEFORE INSERT) forces the column to `unverified` on row creation. The UPDATE guard alone is not enough, for exactly the reason spelled out for `is_admin`: a first-time user has no row yet and the INSERT policy lets them create their own, so they could sign up already `verified` and never touch an UPDATE.

  Both exempt `service_role` — i.e. `stripe-identity-webhook` is the only thing that can grant `verified`, which is what this file already claimed was true. Both also exempt a **NULL `auth.role()`**, which only happens on a direct privileged connection (dashboard SQL editor, psql, MCP) — deliberately, so correcting a status by hand still works. Making that case fail-secure instead would silently revert dashboard edits with no error, and buys nothing: anyone on a direct connection can drop the triggers anyway. The attack surface being closed is PostgREST, where `auth.role()` is always populated.

  **A reverted write is silent — no error reaches the client.** So if you add a new legitimate transition, it must go through the webhook (or another `service_role` path), not through `UserProfileRepository.upsert`.
...
- **Two triggers enforce that, and you need both.** `prevent_self_admin_escalation` (BEFORE UPDATE) reverts any client-side change to `is_admin`; `force_is_admin_false_on_insert` (BEFORE INSERT) forces it false on row creation. The UPDATE guard alone isn't enough — a first-time user has no row yet and the INSERT policy lets them create their own, so they could sign up with `is_admin` already true and never touch an UPDATE. Both exempt `service_role` only. The same two-trigger shape guards `verification_status` — see the Database section above.
...
- **Feature flags are remote** (`public.feature_flags`, `FeatureFlagRepository`), not local/per-device — changing one from the dev menu changes behavior for every user, not just the admin's. RLS: any authenticated user can read (onboarding needs to check flags before it's known whether *that* user is an admin), only admins can write, via a policy that checks the caller's own `users.is_admin` — no security-definer helper needed here since (unlike the anon web RPCs above) the caller already has standing SELECT access to their own row.
- **A flag is three-way, not a boolean**: `feature_flags.audience` is `all` (on for everyone), `admin` (on only where `users.is_admin`), or `off` — a `text` column with the `feature_flags_audience_check` constraint, mirrored by `FeatureFlagAudience`. `admin` is the dark-launch setting: exercise a change against real production data before the whole user base gets it. `FeatureFlagAudience.includes(isAdmin:)` does the resolution and `AppState.hydrate` fetches `isAdmin` *before* the flags, so the two can't be out of step. The dev menu picker reads `AppState.audience(for:)` — the raw stored value — while everything else reads the resolved `is…Enabled` properties; for an `admin` flag those two deliberately differ.
- **`feature_flags.enabled` still exists but is now generated** (`audience = 'all'`) and rejects writes. It's kept purely for builds shipped before `audience` existed: they decode that column, and a missing key throws out of `FeatureFlagRepository.fetchAll` → `AppState.hydrate` → sign-in, which is the same failure mode as the `unverified` bug above. Such a build reads `admin` as `false`, the conservative reading, since it can't do the admin check itself. Drop the column only once no build in the wild reads it.
- **Every flag now falls back to `off`** when the server has nothing this build can use — no row, flags not loaded, or an unrecognised audience — but `FeatureFlagKey.fallbackAudience(for:)` deliberately stays a per-flag switch rather than collapsing to one default: the safe direction belongs to what a flag guards, and `stripe_identity_verification` used to fall back to `all` for exactly that reason. An unknown key falls back to `off`. Current flags:
  - `stripe_identity_verification` (`FeatureFlagKey.stripeIdentityVerification`, dev menu: **"Show Stripe Identity Verification"**) — **currently `off`.** Off ⇒ `OnboardingFlowView`'s `.verify` step skips `IdentityVerificationView` entirely and goes through `skipVerification()`, which sets `verification_status` to `unverified` explicitly and advances — the identical path a manual **Skip** takes, which is the point: the two ways past the step must not differ. (The write is only ever `unverified` over `unverified` in practice — `prevent_self_verification_escalation` reverts anything else a client attempts — so it asserts the resting state rather than being a downgrade path.) Falls back to **`off`**: since verification became optional, a build that can't read the flag showing the step to *nobody* is the conservative reading, and Settings offers it again later.
  - `stripe_identity_test_mode` (`FeatureFlagKey.stripeIdentityTestMode`, dev menu: **"Use Test Environment for Stripe Identity Verification"**) — **currently `off`.** On ⇒ that user's VerificationSessions are created against Stripe's **test** environment instead of live. **Admin-only in three independent places**, because a sandbox verification proves nothing about a real person and must never be what a real user gets: `FeatureFlagPolicy.rejectionReason(setting:to:)` makes the dev-menu picker refuse `all` outright; `AppState.isStripeIdentityTestModeEnabled` ANDs in `isAdmin`, so a value written around that UI still can't resolve on for a non-admin; and — the only one that actually binds — `stripe-identity-session` re-reads both the flag and `users.is_admin` server-side and picks the Stripe key itself, so the client never gets a say. The webhook adds a fourth check at the write end: a `livemode: false` event is ignored unless its target user is an admin. Falls back to **`off`**; you never want to fall *into* the sandbox.
