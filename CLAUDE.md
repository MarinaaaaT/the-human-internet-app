...
Server-side signing already exists as an alternative path: `Verification/RemotePhotoSigner.swift` calls the `sign-photo` Supabase Edge Function (in `the-human-internet-backend`), which forwards to an AWS Lambda signing with a key that lives entirely in AWS KMS and never ships in the app bundle. Gated behind the `aws_server_side_signing` feature flag, switched in `Camera/CameraCaptureView.swift`. Still uses the same dev cert chain, so `signingCredential.untrusted` applies there too until the Conformance Program is complete.
