...
- **`gh` auth works from this session now** (`gh auth status` is green, `MarinaaaaT` account, `repo` scope) — `gh workflow run` / `gh run view` / `gh secret list` all work directly. **`git push` still doesn't** — no stored credential, fails with "could not read Username ... Device not configured." Every commit this session was made locally and needed a manual `git push` from Marina's own terminal.
